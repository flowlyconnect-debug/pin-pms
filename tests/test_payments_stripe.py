...
def test_create_checkout_with_vat_breakdown_passes_correct_line_items_to_stripe(
    ...
    def fake_create(**kwargs):
        ...
        return SimpleNamespace(id="cs_test", payment_intent="pi_test", url="https://stripe", expires_at=None)
...
def test_webhook_succeeded_marks_invoice_paid_and_publishes_event(
    ...
    db.session.commit()
    published = []
    monkeypatch.setattr(payment_services, "publish_webhook_event", lambda *args, **kwargs: published.append(args))
    monkeypatch.setattr(payment_services, "send_template", lambda *args, **kwargs: True)
    with app.app_context():
        payment_services.handle_webhook_event(
            "stripe",
            {"type": "payment.succeeded", "provider_session_id": "cs_1", "provider_payment_id": "pi_1", "method": "card"},
        )
...
def test_refund_partial_then_full(app, organization, regular_user, admin_user, monkeypatch):
    ...
    with app.app_context():
        payment_services.refund(payment.id, "30.00", "partial", actor_user_id=admin_user.id, idempotency_key="r-1")
        payment_services.handle_webhook_event(
            "stripe",
            {"type": "refund.succeeded", "provider_payment_id": "pi_ref_1", "provider_refund_id": "re_1"},
        )
    db.session.refresh(payment)
    assert payment.status == "partially_refunded"
    # second and third refunds
    with app.app_context():
        payment_services.refund(payment.id, "70.00", "final", actor_user_id=admin_user.id, idempotency_key="r-2")
        # newest pending refund gets completed
        pending = PaymentRefund.query.filter_by(payment_id=payment.id, status="pending").first()
        payment_services.handle_webhook_event(
            "stripe",
            {"type": "refund.succeeded", "provider_payment_id": "pi_ref_1", "provider_refund_id": pending.provider_refund_id},
        )
    db.session.refresh(payment)
...
def test_refund_exceeding_amount_returns_400(app, organization, regular_user, admin_user):
    ...
    with app.app_context():
        try:
            payment_services.refund(payment.id, "20.00", "too much", actor_user_id=admin_user.id, idempotency_key="x-1")
            assert False, "expected PaymentServiceError"
        except payment_services.PaymentServiceError as err:
            assert err.status == 400


def test_refund_idempotency_with_key_conflict(app, organization, regular_user, admin_user, monkeypatch):
    inv = _invoice(organization, regular_user)
    payment = Payment(
        organization_id=organization.id,
        invoice_id=inv.id,
    ...
    monkeypatch.setattr(
        payment_services,
        "get_provider",
        lambda _name: SimpleNamespace(refund=lambda **_kwargs: {"provider_refund_id": "re_same", "status": "pending"}),
    )
    with app.app_context():
        first = payment_services.refund(payment.id, "1.00", "a", actor_user_id=admin_user.id, idempotency_key="same-key")
        second = payment_services.refund(payment.id, "1.00", "a", actor_user_id=admin_user.id, idempotency_key="same-key")
        assert first["refund_id"] == second["refund_id"]
        try:
            payment_services.refund(payment.id, "2.00", "b", actor_user_id=admin_user.id, idempotency_key="same-key")
...
def test_refund_provider_exception_marks_failed(app, organization, regular_user, admin_user, monkeypatch):
    inv = _invoice(organization, regular_user)
    payment = Payment(
        organization_id=organization.id,
        invoice_id=inv.id,
    ...
    monkeypatch.setattr(
        payment_services,
        "get_provider",
        lambda _name: SimpleNamespace(refund=lambda **_kwargs: (_ for _ in ()).throw(RuntimeError("boom"))),
    )
    with app.app_context():
...
def test_stripe_parse_webhook_event_variants():
    ...
    completed = provider.parse_webhook_event(
        payload={"type": "checkout.session.completed", "data": {"object": {"id": "cs_1", "payment_intent": "pi_1", "amount_total": 1000, "currency": "eur"}}}
    )
    failed = provider.parse_webhook_event(
        payload={"type": "payment_intent.payment_failed", "data": {"object": {"id": "pi_2", "currency": "eur", "last_payment_error": {"message": "declined"}}}}
    )
    refunded = provider.parse_webhook_event(
        payload={"type": "charge.refunded", "data": {"object": {"id": "ch_1", "payment_intent": "pi_1", "amount_refunded": 500, "currency": "eur"}}}
    )
...
def test_stripe_refund_uses_payment_intent_or_charge(app, monkeypatch):
    ...
    monkeypatch.setattr(stripe_provider_mod, "_stripe", lambda: fake_stripe)
    with app.app_context():
        provider.refund(provider_payment_id="pi_123", amount=Decimal("1.00"), reason="a", idempotency_key="k1")
        provider.refund(provider_payment_id="ch_123", amount=Decimal("1.00"), reason="a", idempotency_key="k2")
